...
def monitor():
    ...
    app_name = os.environ.get("APP_NAME", "Defaulty")
...
def release():
    ...
    reply_markup = {
        "inline_keyboard": [
            [
                {"text": "Direct", "url": f"https://github.com/{repo}/releases/latest/download/Defaulty.apk"},
                {"text": "Versioned", "url": f"https://github.com/{repo}/releases/download/v{version}/Defaulty-v{version}.apk"}
            ]
        ]
    }
...
def changelog():
    repo = os.environ.get("REPOSITORY", "tanvirr007/defaulty-app")
...
def ota():
    ...
    download_url = f"https://github.com/{repo}/releases/download/{full_version_name}/Defaulty.apk"

    file_size = None
    for candidate in (
        f"app/build/outputs/apk/release/Defaulty-v{version_name}.apk",
        "app/build/outputs/apk/release/Defaulty.apk",
